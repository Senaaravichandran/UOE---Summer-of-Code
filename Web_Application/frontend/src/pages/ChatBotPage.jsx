...
function ChatBotWindow() {
  ...
  const handleCameraSelect = async (event) => {
    ...
    if (selectedPlace) {
      // Clear chat history when changing camera
      setChatHistory([]);
      ...
      try {
        ...
        // Update vector store
        const response = await axios.post(`${flask_url}/updateVectorStore`, {
          place: selectedPlace,
        });
        ...
        console.log("Vector store updated:", response.data);

      } catch (error) {
        console.error("Error updating vector store:", error);
        const errorMsg = error.response?.data?.error || "Failed to update vector store. Please try again";
  ...
  const handleSendMessage = async () => {
    ...
    if (!selectedCamera) {
      setChatHistory((prev) => [
        ...
        { sender: "chatbot", message: "⚠️ Please select a camera feed first before asking questions!" }
      ]);
      setUserMessage("");
      return;
    }
    ...
    try {
      ...
      console.error("Error:", error);
      const errorMessage = error.response?.data?.error || "Sorry, I encountered an error. Please try again or select the camera again.";
      setChatHistory((prev) => [
        ...prev,
        {
          sender: "chatbot",
        ...
        <div className="flex-1 flex flex-col items-center mt-3">
          <div className="text-center mb-6">
            <div className="text-4xl lg:text-4xl  tracking-wider">
              <span className="bg-gradient-to-tr from-[#00FF00] via-[#00E68F] to-[rgba(0,230,143,0.5)] bg-clip-text text-transparent font-semibold">
                Hi, Iam CCTV Bot
              </span>{" "}
            </div>
              ...
              >
                {chatHistory.length === 0 ? (
                  ...
                  chatHistory.map((chat, index) => (
                    <div
                      key={index}
                      className={`p-4 rounded-2xl max-w-[80%] ${chat.sender === "user"
          ...
          >
            ...
            <div className="relative" style={{ height: "400px" }}>
              ...
              {selectedCamera && (
                ...
                >
                  <div className="w-2 h-2 rounded-full bg-[rgba(0,230,143,0.5)] animate-pulse"></div>
                  <span>REC</span>
                </div>
              )}
            </div>
        ...
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl text-white">
              <span className="text-[#00E68F] font-semibold">
                Chatbot
              </span>{" "}
